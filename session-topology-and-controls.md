...
| Bias generated workflows smaller | `workflowSizeGuideline: "small"` in any settings file (v2.1.219+, takes precedence over `/config` and hides that row) — or `/config` → Dynamic workflow size. Sizes: small <5 · **medium <10 (v2.1.271; was <15)** · large <50. Default `medium`, or **`small` on Pro plans** (v2.1.271+) |
| Warn earlier on big workflows | `Large workflow` warning fires at >25 scheduled agents or >1.5M projected tokens — ✅ documented (workflows.md, v2.1.203+). **Choosing a size guideline replaces the 25-agent threshold with that guideline's count** (an explicit `medium` → warns at >10 since v2.1.271). Sessions with **ultracode** on are exempt and see no warning |
| Workflow hits the usage limit | v2.1.271+: *"the run pauses rather than failing that agent … Shortly after the limit resets, the waiting agents run again and the run continues on its own"* — gated by `autoContinueAtUsageLimit` (turning it off mid-wait fails the waiting agents). Another unattended resume-and-burn path — ✅ confirmed (workflows.md) |
| Raise a workflow run's concurrent-agent limit | `CLAUDE_CODE_WORKFLOW_MAX_CONCURRENT_AGENTS` (1–256, v2.1.269+) — changelog only, not yet in workflows.md/env-vars.md (which still document "Up to 16 concurrent agents"). Raising it raises burn *rate*; leave unset |
| Keep CLAUDE.md out of a subagent's prompt | `omitClaudeMd: true` in agent frontmatter or `--agents` JSON (v2.1.271+) — skips user/project/local CLAUDE.md (managed policy files still load). Only the **built-in** Explore/Plan skip CLAUDE.md by default; a custom agent — **including a user-level override named `Explore`** — loads the full hierarchy on every spawn — ✅ confirmed (sub-agents.md) |
...
| Cap the effort level (and thereby thinking spend) | `maxEffortLevel` (v2.1.267+) — `"low"`/`"medium"`/`"high"`/`"xhigh"`/`"max"`, default unset. Caps effort *"on every provider, including Bedrock, Vertex and Foundry"*; anything higher runs at the cap, including `/effort`, the `/model` picker, `--effort`, `CLAUDE_CODE_EFFORT_LEVEL`, and a skill's or subagent's `effort` frontmatter. Scope `Any file`, **lowest cap across scopes wins** — a lower scope can't raise it, so managed settings can enforce it — ✅ confirmed (settings-reference.md) |
| Make ultracode unavailable | Set `maxEffortLevel` below `xhigh`: *"a cap below `xhigh` makes ultracode unavailable on the models the cap applies to"* (v2.1.267+). This is the only documented lever — ultracode is otherwise exempt from the concurrency cap and from the large-workflow warning |
| Exempt one model from an effort cap | `maxEffortLevel: "max"` inside that model's `modelSettings` entry — replaces the top-level key **only within the same settings source**, so it can't escape a stricter managed source |
