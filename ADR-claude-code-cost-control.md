...
| Option | Verdict | Why |
|---|---|---|
| `CLAUDE_CODE_SUBAGENT_MODEL` as a global default | **Rejected** | It is the **highest-precedence override**, not a default (model-config.md; resolution order in sub-agents.md). Set globally it downgrades your opus reviewers and upgrades your haiku explorers, discarding the roster. There is no "soft default" settings knob. |
...
| Tactical per-session env override | **Kept as a tool** | `CLAUDE_CODE_SUBAGENT_MODEL=haiku` for *one* heavy session to force all workers cheap is legitimate — just never as a persisted global. |
...
| # | Finding | Source |
|---|---|---|
| 1 | `CLAUDE_CODE_SUBAGENT_MODEL` overrides per-invocation model AND frontmatter; `inherit`==unset (v2.1.196+); full resolution order env > param > frontmatter > session | code.claude.com/docs/en/model-config, /sub-agents |
| 2 | Statusline JSON exposes `rate_limits.{five_hour,seven_day}.used_percentage` + `resets_at`, `cost.total_cost_usd`, `context_window.used_percentage`, `exceeds_200k_tokens`, `effort.level`, `agent.name`; Pro/Max only, after first response; ANSI supported; stdout captured (not a tty) | code.claude.com/docs/en/statusline |
| 3 | Workflow agents inherit the session model unless a stage sets `opts.model`; `/config` Dynamic workflow size = unrestricted/small<5/medium<15/large<50; runtime caps 16 concurrent / 1000 total | code.claude.com/docs/en/workflows |
| 4 | **Spawn surface:** a subagent spawn is a PreToolUse call on the `Agent` tool (`Task` alias; renamed v2.1.63); PreToolUse blocks via exit 2 or JSON `permissionDecision:"deny"` and is the only blocking surface; `SubagentStart` cannot block; `TaskCreated` = task-list event | code.claude.com/docs/en/hooks, /sub-agents, /tools-reference |
| 5 | Telemetry: `CLAUDE_CODE_ENABLE_TELEMETRY=1` + OTLP; metrics `claude_code.token.usage`, `.cost.usage`, `.session.count`; `CLAUDE_CODE_ENHANCED_TELEMETRY_BETA=1` emits spans; `agent.name`/`query_source` attrs | code.claude.com/docs/en/monitoring-usage |
...
| 11 | Whether settings.json PreToolUse fires for subagent-originated tool calls is AMBIGUOUS in docs (`agent_id` "present only when the hook fires inside a subagent call" vs frontmatter hooks positioned as the per-agent mechanism) → frontmatter replication covers nested spawns either way; frontmatter `hooks:` is a documented field (plugin agents excepted) | code.claude.com/docs/en/hooks, /sub-agents |
| 12 | Subagent frontmatter fields incl. `hooks`, `model` (sonnet/opus/haiku/fable/full-ID/inherit), `effort`, `background`, `isolation: worktree`, `maxTurns`; nested spawns v2.1.172+, depth ≤5; background-by-default v2.1.198+ | code.claude.com/docs/en/sub-agents |
